...
def process_file(p):
    ...
    # Find the footer block replacing everything inside <div class="footer-right">...</div> with the new structure
    # and also remove the old social footer if it exists.
    
    # 1. First, extract the links from footer-right
    links_match = re.search(r'<div class="footer-right">\s*(.*?)\s*</div>', content, re.DOTALL)
    if not links_match:
        print("Could not find footer-right in", p)
        return
    links = links_match.group(1)
    
    if 'class="footer-links"' in links:
        print("Already updated", p)
        return

    # 2. Rebuild the footer-right section
    social_html = """
